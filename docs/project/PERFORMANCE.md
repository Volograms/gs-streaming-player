...
| PLY/SPZ tier build  | Public SplatTransform merge-decimation can produce complete bundled SOG or SPZ tiers.            | Current public authoring path; minimum-tier visual quality still requires dataset calibration.                |
| Quality-LoD RAD     | Its tree successfully produced the flat cuts used by the historical measurements.                | Legacy authoring tool only; no longer a dependency of the normal public build.                                |
